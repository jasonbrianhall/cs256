		 ...
		 )
{
	...
	if (res!=0) {
        	if (errno==ENOENT) return(ERR_SUP_INVDIR);
        }
...
int sys_check_program(	char	dir_name[],	/* directory name */
			...
			unsigned int*  prog_len_p,	/* ptr to prog length */
			unsigned int*  start_offset_p	/* ptr to start offset */
			)
{
	...
	unsigned int file_size;	/* size of complete file */
	int header_size;/* size of header */
	unsigned int code_size;	/* size of program code */
	unsigned int data_size;	/* size of program data */
...
int sys_load_program (	void*	load_addr,	/* address for loading */
			unsigned int max_size,	/* memory size */
                        ...
                        )
{
	...
	int errcode;	/* error code */
	unsigned int prog_len;	/* program length */
	unsigned int offset;	/* offset for starting */
